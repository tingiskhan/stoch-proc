...
class LocalLinearTrend(LinearModel):
    ...
    def __init__(self, sigma: ParameterType, initial_mean: ParameterType = None):
        ...
        increment_dist = Normal(
            loc=torch.tensor(0.0, device=sigma.device),
            scale=torch.tensor(1.0, device=sigma.device)
        ).expand(torch.Size([2])).to_event(1)

        a = torch.tensor([[1.0, 0.0], [1.0, 1.0]], device=sigma.device)

        super().__init__(a, sigma, increment_distribution=increment_dist, initial_kernel=initial_kernel, initial_parameters=(initial_mean, sigma))
